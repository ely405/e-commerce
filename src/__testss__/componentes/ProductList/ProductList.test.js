...
import ConnectedProductList, { ProductList } from '../../../components/ProductList/ProductList';
...
it('Renders no products when store is empty', () => {
    ...
    // console.log('it', wrapper.find('.product-container').length);
});
...
it('Renders no products when store is empty, connected component', () => {
    ...
    expect(wrapper.find('.product-container').length).toBe(0);
});
